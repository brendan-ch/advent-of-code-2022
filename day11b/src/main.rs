        ...
        for i in 0..monkeys.len() {
            while monkeys[i].items.len() > 0 {
                ...
                match monkeys[i].operation.operator {
                    '+' => monkeys[i].items[0] = (
                        if monkeys[i].operation.left == "old" { item } else { monkeys[i].operation.left.parse::<i128>().unwrap() }
                    ) + (
                        if monkeys[i].operation.right == "old" { item } else { monkeys[i].operation.right.parse::<i128>().unwrap() }
                    ),
                    '*' => monkeys[i].items[0] = (
                        if monkeys[i].operation.left == "old" { item } else { monkeys[i].operation.left.parse::<i128>().unwrap() }
                    ) * (
